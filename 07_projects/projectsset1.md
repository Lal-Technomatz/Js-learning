...
function checkGuess(guess) {
  if (guess === randomNumber) {
    displayMessage(`you gussesed it Right`);
    endGame();
  } else if (guess < randomNumber) {
    displayMessage(`The Number is Lower then the Actual Number`)
  }else if (guess > randomNumber) {
    displayMessage(`The Number is Higher then the Actual Number`)
  }
}
...
const startChangingColor = function () {
  if (!intervalId) {
    intervalId = setInterval(changeBgColor, 1000)                //setInterval(function_reference, 1000)
  }
